...
Contract for assets transfers. The user can transfer the asset to any of the approved chains. The admin manages the assets, fees and other important
protocol parameters.
...
```
Maps `debridgeId` (see `getDebridgeId`) to threshold amount after which `Math.max(excessConfirmations,SignatureVerifier.minConfirmations)` is used instead
...
This method is used for the transfer of assets from the [native
chain](/dePort/transfer-flow#transfers-from-native-source-chain). It locks an asset in the smart contract in the native
chain and enables minting of deAsset on the secondary chain.
...
Is used for transfers [into the native chain](/dePort/transfer-flow#transfers-from-native-source-chain)

...
Orders are identified by a [deterministic order ID](/protocol-specs/deterministic-order-id), and the hash of the hook's raw data is essentially a part
of this ID (see the last two fields of the [deterministic order ID](/protocol-specs/deterministic-order-id)), so once a user signs-off an order with a

...
Orders are identified by a [deterministic order ID](dln-dropdown/protocol-specs/deterministic-order-id), and the hash of the hook's raw data is essentially a part
of this ID (see the last two fields of the [deterministic order ID](dln-dropdown/protocol-specs/deterministic-order-id)), so once a user signs-off an order with a
...
The proper execution of the hook during order fulfillment is guaranteed by DlnDestination — the DLN smart contract responsible for filling orders,
...
solver and fill any open order. The  DlnDestination smart contract simply ensures that the requested amount is provided in full and further forwarded
either to the recipient or to a hook's target (via the DlnExternalCallAdapter smart contract). This means that a hook's target should be designed with
...
users to initiate a cancellation procedure. This increases friction and worsens the overall user experience, so it is advised to carefully test hooks,
and estimate potential fulfillments before placing orders with such hooks in production. The API takes the burden of proper hook data validation,
...
Further reading:
- Easy usage with the DLN API: Integrating deBridge hooks
- Technical specification: Hook data
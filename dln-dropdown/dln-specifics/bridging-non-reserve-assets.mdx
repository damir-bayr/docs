...
The [DeBridge Liquidity Network (DLN)](/overview/introduction) protocol supports bridging any liquid token from the source 
chain to the destination chain,  not just [reserve assets](/dln-specifics/reserve-assets). While the actions taken by the system are similar to those used 
when [bridging reserve assets](/dln-specifics/bridging-reserve-assets), the internal flow differs and introduces considerations 
...
Cross-chain settlements are executed in [reserve assets](/dln-specifics/reserve-assets) to simplify operations for solvers. 
...
<Tip>
  If integrators prefer to manage the swap path manually, they can perform the conversion to [reserve assets](/dln-specifics/reserve-assets) 
...
Once the swap completes, the [reserve assets](/dln-specifics/reserve-assets) are locked within the protocol until the order 
is either [fulfilled]() or [cancelled](). 

<Tip>
  If the order is [cancelled](), the locked [reserve assets](/dln-specifics/reserve-assets) are returned to the user. 

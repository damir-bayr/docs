...
The simplest scenario occurs when bridging [reserve assets](dln-dropdown/dln-specifics/reserve-assets) from the source chain to the destination chain. 
...
- Step 2: After receiving the [response](), call `approve` on the ERC-20 contract of the [reserve assets](dln-dropdown/dln-specifics/reserve-assets). 
...
- Step 3: Sign response.tx and submit the transaction. This action locks the specified amount of [reserve assets](dln-dropdown/dln-specifics/reserve-assets) 
...
chain are available [here](dln-dropdown/dln-specifics/order-fulfillment/order-fulfillment).
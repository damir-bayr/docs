...
## **`tx`**
  - `data`: The data that must be signed and submitted. It contains all necessary information to initiate a [cross-chain
    order](/dln-specifics/bridging-reserve-assets), including cases involving [non-reserve assets](/dln-specifics/bridging-non-reserve-assets). It is
  ...
  - `value`: [Flat fee](/overview/fees-supported-chains) in the source chain's native currency charged by the DLN protocol. Applicable to EVM source chains only.
...
## **`orderId`**  
  ...
  status](/integration-guidelines/api/monitoring-orders/rest).

## **`fixFee`**  
  [Flat fee](/overview/fees-supported-chains) charged in the source chain's native currency. This matches `tx.value` for EVM-based chains.

...
The DLN API provides a convenient high-level interface to attach hooks to orders upon requesting an order creation transaction. The API takes the
...
hook execution would prevent an order from getting filled, and an order's authority would need to initiate a cancellation procedure from the
destination chain, which increases friction and worsens UX. 

To specify the hook, use the `dlnHook` parameter of the `create-tx` endpoint. The value for this parameter must be a JSON object that describes the
...
To craft a proper versioned transaction, use the [Creating Hook data for Solana](integration-guidelines/api/hooks/solana-hook-data) guide.

<Accordion title="DlnHook encoding example">
```typescript
...
hook](dln-dropdown/protocol-specs/evm-chains-hook-anatomy#universal-hook), the DLN API provides a simple shortcut for that:
...
The `data.to` and `data.calldata` properties represent the transaction call that should be made, as explained in the Universal hook section. The gas
...
<Accordion title="Deposit 0.1 USDC to AAVE Hook Example"> 
...
[HookDataV1](dln-dropdown/protocol-specs/evm-chains-hook-anatomy) struct:
...
an order's authority to initiate a cancellation procedure from the destination chain, which increases friction and worsens UX. 

...
- `_referralCode` can be set to zero (`0`); it is an invitation code to identify your transaction. If you don't have it, you can get one by pressing the
  INVITE FRIENDS button at app.debridge.finance. Governance may thank you later for being an early builder.
...
<Accordion title="Order Preparation">
```solidity
// preparing an order
...
- for the `_executionFee` argument, specify the amount of native blockchain currency (in addition to the deBridge protocol fee) to provide an
  ...
  in the [deExplorer](https://explorer.debridge.finance/) and claim it manually. Consider understanding how the cross-chain call is handled.
